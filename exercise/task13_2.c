//输入五名超女结构体,存放在把结构体数组放在二进制文件中的数据放在struct st_girl数据结构中
#include <stdio.h>
...
int main(){
  ...
  fread(&girls,1,sizeof(girls),fp);
  printf("%d\n%d\n%lf\n%s\n%s\n%s\n",girls[0].age,girls[0].height,girls[0].weight,girls[0].name,girls[0].sc,girls[0].yz);
}
