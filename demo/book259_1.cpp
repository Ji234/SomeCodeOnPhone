	...
	if((sem_id=semget(key,1,0640))==-1){
		...
		if (errno==2){
			if((sem_id=semget(key,1,0640|IPC_CREAT))==-1){
				...
				//after build,it must init to be useful
...
int main(){
	...
	printf("before wirte is %s\n",ptext);
	sprintf(ptext,"the pid is %d",getpid());
	printf("after write is %s\n",ptext);
	sleep(5);
	if(sem.post()==false){
		printf("sem.post is failed\n");
		return -1;
	}
	
	//at the sleep time we can run other 259 application
